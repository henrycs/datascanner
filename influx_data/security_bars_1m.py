...
async def get_security_minutes_data(ft: FrameType, target_date: datetime.date):
    ...
    len2 = len(secs2)
    if len1 != len2:
        logger.error("bars:%s data corrupt, 9:30+%s:00 != 11:30:00, %d, %d", ft.value, ft.value, len1, len2)
        return None
    ...
    len3 = len(secs3)
    if len2 != len3:
        logger.error("bars:%s data corrupt, 11:30:00 != 13:01:00, %d, %d", ft.value, len2, len3)
        return None
    ...
    len4 = len(secs4)
    if len3 != len4:
        logger.info("bars:%s data corrupt, 13:00+%s:00 != 15:00:00, %d, %d", ft.value, ft.value, len3, len4)
        return None
...
async def get_security_minutes_bars(ft: FrameType, start: datetime.datetime, end: datetime.datetime):
