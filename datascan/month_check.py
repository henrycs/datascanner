...
async def scan_bars_1M_for_seclist(target_date: datetime.date, d0: datetime.date):
    start = datetime.datetime.combine(d0, datetime.time(0, 0, 0))
    end = datetime.datetime.combine(target_date, datetime.time(23, 59, 59))

    all_secs_in_bars = await get_security_month_bars(start, end)
...
def get_security_difference(secs_in_bars, all_stock, all_index):
    ...
    if len(y1) > 0:  # 本地数据库中多余的股票或指数
        logger.error("secs in bars:1M but not in jq : %d", len(y1))
        return False

    return True


async def validate_data_bars1M(target_date: datetime.date):
    d0, d1 = TimeFrame.get_frame_scope(target_date, FrameType.MONTH)
    ...
    logger.info("check bars:1M for open/close: %s - %s", d0, d1)
    all_db_secs_data = await scan_bars_1M_for_seclist(d1, d0)
    if all_db_secs_data is None or len(all_db_secs_data) == 0:
        logger.error("failed to get sec list from db for bars:1M/open, %s, %s", d0, d1)
        return False
    ...
    all_jq_secs_data = await get_sec_bars_1M(all_secs_set_jq, d1, d0)
    logger.info(
        "total secs downloaded from bars:1M/open@jq, %d, %s, %s",
