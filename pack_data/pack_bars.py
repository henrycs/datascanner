...
async def get_security_other_bars(ft: FrameType, start: datetime.datetime, end: datetime.datetime):
    client = get_influx_client()
...
async def pack_bars():
    ft = FrameType.WEEK
    file = "/home/app/zillionare/pack_data/bars_1w_2022.pik"
...
async def pack_sec_list():
    file = "/home/app/zillionare/pack_data/seclist_2022.pik"
    
    dt_start = datetime.date(2022, 12, 1)
    dt_end = datetime.date(2022, 12, 31)    
...
async def pack_xrxd_list():
    file = "/home/app/zillionare/pack_data/sec_xrxd_2022.pik"
    
    dt_start = datetime.date(2022, 1, 1)
    # last trade day, --> day_shitf(0)
    dt_end = datetime.date(2023, 2, 28)

    secs_data = await get_xrxd_list(dt_start, dt_end)    
    if secs_data is None or len(secs_data) == 0:
        logger.info("pack_xrxd_list, failed to get sec xrxd from database")
        os._exit(1)
    logger.info("secs xrxd reports downloaded: %s - %s, %d", dt_start, dt_end, len(secs_data))
