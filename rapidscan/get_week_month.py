...
from jq_fetcher import get_sec_bars_1M, get_sec_bars_1d, get_sec_bars_1w, get_sec_bars_pricelimits
...
async def get_1w_for_price(all_secs_today, target_date: datetime.date, d0: datetime.date, prefix: SecurityType):
    # d0: 本周第一天, target_date：本周最后一天

    # download all data from jq
    all_secs_data = await get_sec_bars_1w(all_secs_today, target_date, d0)
    logger.info("total secs downloaded from bars:1w@jq, %d, %s", len(all_secs_data), prefix)

    if len(all_secs_data) == 0:
        logger.error("failed to get price data from bars:1w@jq, %s (%s)", target_date, prefix)
        return False

    await Stock.persist_bars(FrameType.WEEK, all_secs_data)
    logger.info("get from bars:1w@jq and saved into db, %s, %s, %d", prefix, FrameType.WEEK, len(all_secs_data))
...
async def get_1M_for_price(all_secs_today, target_date: datetime.date, d0: datetime.date, prefix: SecurityType):
    # d0: 本月第一天, target_date：本月最后一天

    # download all data from jq
    all_secs_data = await get_sec_bars_1M(all_secs_today, target_date, d0)
    logger.info("total secs downloaded from bars:1M@jq, %d, %s", len(all_secs_data), prefix)

    if len(all_secs_data) == 0:
        logger.error("failed to get price data from bars:1M@jq, %s (%s)", target_date, prefix)
        return False

    await Stock.persist_bars(FrameType.MONTH, all_secs_data)
    logger.info("get from bars:1M@jq and saved into db, %s, %s, %d", prefix, FrameType.MONTH, len(all_secs_data))
