...
async def remove_secs_from_bars_1d(
    all_secs_today, target_date: datetime.date, secs_closed, secs_limits
):
    ...
    logger.info(
        "total secs to be removed from bars:1d:limit@db, %d", len(secs_to_be_removed2)
    )
...
async def scan_1d_for_close_price(
    all_secs_today, target_date: datetime.date, secs_in_bars: set
):
    ...
    logger.info(
        "total secs to be retrieved from jq (bars:1d), %d", len(secs_to_be_added)
    )
    ...
    if len(secs_to_be_added) > 0:
        ...
        if len(all_secs_data) > 0:
            ...
            logger.info(
                "secs downloaded from bars:1d@jq and saved into db, %s, %d",
                FrameType.DAY,
                len(all_secs_data),
            )
...
async def scan_1d_for_pricelimits(
    all_secs_today, target_date: datetime.date, secs_in_bars: set
):
    # 当日证券列表有，日线数据没有，可能的情况：
    # 缺失（要补录），可能是停牌（需要判断返回的数据日期），可能是停牌前的整理时间已到（无数据）
    ...
    logger.info(
        "total secs to be retrieved from jq (bars:1d:limit), %d", len(secs_to_be_added)
    )
    ...
    if len(secs_to_be_added) > 0:
        ...
        logger.info(
            "total secs downloaded from bars:1d:limits@jq, %d", len(all_secs_data)
        )
        if len(all_secs_data) > 0:
            # await Stock.save_trade_price_limits(all_secs_data, to_cache=False)
            logger.info(
                "secs downloaded from bars:1d:limits@jq and saved into db, %s, %d",
                FrameType.DAY,
                len(all_secs_data),
            )
